...
import { login, qr } from '../api/api'
import { useParams } from 'react-router-dom'


const QRLogin = () => {

    const {scooterID, method} = useParams()
    ...
    const handleClick = async () => {
        try {
            ...
            if (user.role === 'customer'){
                await qr(scooterID, method, user.id)
            }

            window.alert(`Scooter successfully ${method}`)
                ...
                <div className='flex justify-start items-center w-full pl-10'>
                    ...
                    <div className='w-2/3'>
                        <button 
                          ...
                          onClick={handleClick}
                        >
                            <span className='capitalize'>{method}</span> Scooter
                        </button>
                    </div>
                </div>
                </div>
